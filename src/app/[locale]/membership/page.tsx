...
export default async function MembershipPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <PageHeader title={pageTitle} subtitle={pageContent} backgroundColor={membershipPage?.background_color} gradientTo={membershipPage?.banner_gradient_to} textColor={membershipPage?.text_color} width={membershipPage?.banner_width} height={membershipPage?.banner_height} />
      <MembershipClient 
        plans={plans} 
        pageTitle={pageTitle} 
